...
/*-----------------------------------------------------------------------
   Modified by Mahdi Tavakol (NUIG) to do SMD in a specific direction 
...
enum { SMD_NONE=0,
       SMD_TETHER=1<<0, SMD_COUPLE=1<<1, SMD_DIRECTION=1<<7,
  ...
  Fix(lmp, narg, arg)
{
  ...
  if (strcmp(arg[argoffs],"tether") == 0) {
	...
	xc = force->numeric(FLERR,arg[argoffs+1]);
	yc = force->numeric(FLERR,arg[argoffs+2]);
    zc = force->numeric(FLERR,arg[argoffs+3]);
    r0 = force->numeric(FLERR,arg[argoffs+4]);
	xflag = 0;
	yflag = 0;
	zflag = 0;
